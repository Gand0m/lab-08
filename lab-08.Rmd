...
weather_noNA <- weather [!is.na(weather$RainTomorrow), ]

count(weather_noNA,RainTomorrow)
...
rainy_days_by_city <- weather %>%
  filter(RainToday == "Yes") %>%        
  group_by(Location) %>%
  arrange(desc(RainToday))  
             
count(rainy_days_by_city) %>%
  arrange(desc(n))
